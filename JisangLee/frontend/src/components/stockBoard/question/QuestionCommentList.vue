        ...
        <v-form @submit.prevent="onCommentSubmit">
            <table>
                <v-row justify="center">
                    ...
                    <v-row v-for="commentList in questionComments" 
...
export default {
    ...
    computed: {
        ...mapState(['QuestionCommentsList'])
    },
    methods: {
        ...mapActions(['fetchQuestionCommentsList']),
        comDelete (commentId) {
            axios.delete(`http://localhost:7777/questionboard/comment/${commentId}`, {commentId})
                    .then(() => {
                        swal({
                            text: "댓글이 삭제되었습니다!",
                            icon: "success"
