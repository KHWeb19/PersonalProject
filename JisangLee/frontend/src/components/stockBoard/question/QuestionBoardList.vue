<template>
        ...
        <v-row column wrap justify="center">
            <v-data-table style="width: 75%" dark dense v-if="!questionBoards || (Array.isArray(questionBoards) && questionBoards.length === 0)"
            :headers="headerTitle">
            </v-data-table>      
            <v-data-table v-else 
                        ...
                        <template v-slot:[`item.title`]="{ item }">
                            <router-link id=sj style=text-decoration:none; :to="{ name: 'QuestionBoardReadPage',
