        ...
        <v-form @submit.prevent="onCommentSubmit">
            <table>
                <v-row justify="center">
                    ...
                    <v-row v-for="commentList in complaintComments" 
...
export default {
    ...
    computed: {
        ...mapState(['ComplaintCommentsList'])
    },
    methods: {
        ...mapActions(['fetchComplaintCommentsList']),
        comDelete (commentId) {  
            axios.delete(`http://localhost:7777/complaintboard/comment/${commentId}`, {commentId})
                    .then(() => {
                        swal({
                            text: "댓글이 삭제되었습니다!",
                            icon: "success"
