        ...
        <v-form @submit.prevent="onCommentSubmit">
            <table>
                <v-row justify="center">
                    ...
                    <v-row v-for="commentList in localComments" 
...
export default {
    ...
    computed: {
        ...mapState(['LocalCommentsList'])
    },
    methods: {
        ...mapActions(['fetchLocalCommentsList']),
        comDelete (commentId) {
            axios.delete(`http://localhost:7777/localboard/comment/${commentId}`, {commentId})
                    .then(() => {
                        swal({
                            text: "댓글이 삭제되었습니다!",
                            icon: "success"
