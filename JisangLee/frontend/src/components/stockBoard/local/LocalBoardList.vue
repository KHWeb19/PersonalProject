<template>
        ...
        <v-row column wrap justify="center">
            <v-data-table style="width: 75%" dark dense v-if="!localBoards || (Array.isArray(localBoards) && localBoards.length === 0)"
            :headers="headerTitle">
            </v-data-table>
            <v-data-table v-else
                        ...
                        :headers="headerTitle" 
                        :items="localBoards"
                        :key="localBoards.boardNo"
                        :items-per-page="10"
                        :search="search"
                        class="elevation-1">
                        <template v-slot:[`item.title`]="{ item }">
                            <router-link id=sj style=text-decoration:none; :to="{ name: 'LocalBoardReadPage',
