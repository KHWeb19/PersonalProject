...
export default {
  ...
  methods: {
    onSubmit (payload) {
      ...
      const { id, sn, pw, pwConfirm } = payload //이게 중괄호로 묶어서 개체 형태로 전달하는 거네
      //axios.post(url[, data[, config]]) 입력하기
      axios.post('http://localhost:7777/vueJpaMember/register', { id, sn, pw, pwConfirm }) //Spring Controller에 연동하여 전달
