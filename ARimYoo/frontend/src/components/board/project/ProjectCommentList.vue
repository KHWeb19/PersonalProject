        ...
        <v-form @submit.prevent="onCommentSubmit">
            <table>
                <v-row justify="center">
                    ...
                    <v-row v-for="commentList in projectComments" 
...
export default {
    ...
    methods: {
        comDelete (commentId) {
            ...
            axios.delete(`http://localhost:7777/board/project/comment/${commentId}`, {commentId})
                    .then(() => {
                        alert('댓글이 삭제되었습니다.')
                        history.go(0)
                    })
                    .catch(() => {
                        alert('삭제 실패! 문제 발생!')
                        console.log(commentId, this.projectNo)
                    })
