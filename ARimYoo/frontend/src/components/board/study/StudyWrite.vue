...
export default {
    ...
    methods: {
        ...
        onStudySubmit () {
            ...
            const { studyName, writer, content, people, openLink, memberNo} = this
            const file =  this.$refs.files.files[0]
             var result = confirm('한번 스터디를 만드시면 모집글 외 수정 불가능합니다.')
             if (result) {
            this.$emit('submit', { studyName, content,writer, people, file, openLink, memberNo })
            console.log(studyName,content,writer,file,people,openLink, memberNo)
             }
