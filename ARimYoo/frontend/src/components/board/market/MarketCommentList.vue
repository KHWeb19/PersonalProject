        ...
        <v-form @submit.prevent="onCommentSubmit">
            <table>
                <v-row justify="center">
                    ...
                    <v-col cols="8">
                        <v-textarea color="red darken-3" cols="76" rows="1" v-model="comment"/>
                    </v-col>
                    <v-col cols="1">
                        <v-btn  type="submit" class="commentBtn" color="red darken-3" dark>input</v-btn>
                    </v-col>
                </v-row>
                    <v-row v-for="commentList in marketComments" 
...
export default {
    ...
    methods: {
        comDelete (commentId) {
            ...
            axios.delete(`http://localhost:7777/board/market/comment/${commentId}`, {commentId})
                    .then(() => {
                        alert('댓글이 삭제되었습니다.')
                        history.go(0)
                    })
