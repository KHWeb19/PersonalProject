...
export default {
  ...
  methods: {
    ...mapActions([ 'fetchCheckBrandComments' ]),
    addComment () {
      const id = this.userInfo.id
      const content = this.content
      const boardNo = this.boardNo

      axios.post('http://localhost:7777/BrandCheck/comment/add', { id, content, boardNo }).then(() => {
        alert('댓글이 등록되었습니다!')

        this.fetchCheckBrandComments(boardNo)
